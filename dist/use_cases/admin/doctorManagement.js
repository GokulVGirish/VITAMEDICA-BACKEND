...
class AdminDoctorManagementInteractor {
    ...
    async getDoctorDocs(id) {
        try {
            ...
            const command1 = this.AwsS3.getObjectCommandS3(doctor.documents.certificateImage);
            const command2 = this.AwsS3.getObjectCommandS3(doctor.documents.qualificationImage);
            const command3 = this.AwsS3.getObjectCommandS3(doctor.documents.aadarFrontImage);
            const command4 = this.AwsS3.getObjectCommandS3(doctor.documents.aadarBackImage);
            const certificateImage = await this.AwsS3.getSignedUrlS3(command1, 3600);
            const qualificationImage = await this.AwsS3.getSignedUrlS3(command2, 3600);
    ...
    async rejectDoctor(id, reason) {
        try {
            ...
            const doctor = result.doctor;
            await this.AwsS3.deleteObjectCommandS3(doctor?.documents?.certificateImage);
            await this.AwsS3.deleteObjectCommandS3(doctor?.documents?.qualificationImage);
            await this.AwsS3.deleteObjectCommandS3(doctor?.documents?.aadarFrontImage);
            await this.AwsS3.deleteObjectCommandS3(doctor?.documents?.aadarFrontImage);
    ...
    async getDoctorProfile(id, page, limit) {
        try {
            ...
            if (result.documents) {
                const command1 = this.AwsS3.getObjectCommandS3(result.documents?.certificateImage);
                const command2 = this.AwsS3.getObjectCommandS3(result.documents?.qualificationImage);
                const command3 = this.AwsS3.getObjectCommandS3(result.documents?.aadarFrontImage);
                const command4 = this.AwsS3.getObjectCommandS3(result.documents?.aadarBackImage);
